...
| Version | Date | Notes |
|---------|------|-------|
| v1.0.45 | 2026-06-09 | **Family Constellation "🌐 Globe" mode.** New layout that places each family at its 1000G **population's real geographic origin** on a Blue-Marble Earth sphere (reuses `POP_COORDS` + `latLonToVec3` + `/earth-blue-marble.jpg` from `CohortGlobe`). Families of a population spread on the local tangent plane to form a patch over their region; unknown-population families ring the south pole. Earth spins with the glyphs (same group); smooth lerp transition from the flat grids. `GLOBE_R=38` matches the grid extent so the camera frames all four modes consistently. |
